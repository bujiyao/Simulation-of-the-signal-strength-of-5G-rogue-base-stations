...
def plot_all_bs_signal(T, Rxlev_bs, Rxlev_rbs, NUM_UE, NUM_BS, NUM_RBS):
    ...
    plt.tight_layout()
    plt.savefig('RSRP from All Base Stations.png', bbox_inches='tight', dpi=300)
...
def rogue_bs_data(T, Rxlev_bs, Rxlev_rbs, NUM_UE, NUM_BS, NUM_RBS, attack_periods=None, handover_threshold=3, plot=True):
    ...
    df.to_csv('rogue_bs_dataset.csv', index=False)
    ...
    if plot:
        ...
        plt.tight_layout()
        plt.savefig('Multiple_Rogue_bs_attacks_RSRP.png', bbox_inches='tight', dpi=300)
